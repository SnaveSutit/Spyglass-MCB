 ...
 */
export function attempt<N extends Returnable = AstNode>(
	parser: InfallibleParser<N>,
	src: Source,
	ctx: ParserContext,
): InfallibleAttemptResult<N>
export function attempt<N extends Returnable = AstNode>(
	parser: Parser<N>,
	src: Source,
	ctx: ParserContext,
 ...
 */
export function repeat<CN extends AstNode>(
	parser: InfallibleParser<CN | SequenceUtil<CN>>,
	parseGap?: InfallibleParser<CN[]>,
): { _inputParserIsInfallible: never } & void
export function repeat<CN extends AstNode>(
	parser: Parser<CN | SequenceUtil<CN>>,
	parseGap?: InfallibleParser<CN[]>,
): InfallibleParser<SequenceUtil<CN>>
export function repeat<CN extends AstNode>(
	parser: Parser<CN | SequenceUtil<CN>>,
	parseGap?: InfallibleParser<CN[]>,
): InfallibleParser<SequenceUtil<CN>> | void {
 ...
 */
export function recover<N extends Returnable>(
	parser: InfallibleParser<N>,
	defaultValue: (src: Source, ctx: ParserContext) => N,
): { _inputParserIsInfallible: never } & void
export function recover<N extends Returnable>(
	parser: Parser<N>,
	defaultValue: (src: Source, ctx: ParserContext) => N,
 ...
 */
export function map<NA extends Returnable, NB extends Returnable = NA>(
	parser: InfallibleParser<NA>,
	fn: (res: NA, src: Source, ctx: ParserContext) => NB,
): InfallibleParser<NB>
export function map<NA extends Returnable, NB extends Returnable = NA>(
	parser: Parser<NA>,
	fn: (res: NA, src: Source, ctx: ParserContext) => NB,
...
export function setType<N extends Omit<AstNode, 'type'>, T extends string>(
	type: T,
	parser: InfallibleParser<N>,
): InfallibleParser<Omit<N, 'type'> & { type: T }>
export function setType<N extends Omit<AstNode, 'type'>, T extends string>(
	type: T,
	parser: Parser<N>,
 ...
 * `Failure` when the `parser` returns a `Failure`.
 */
export function validate<N extends AstNode>(
	parser: InfallibleParser<N>,
	validator: (res: N, src: Source, ctx: ParserContext) => boolean,
	message: string,
...
export function stopBefore<N extends Returnable>(
	...
	return (src, ctx): Result<N> => {
		...
		tmpSrc.string = tmpSrc.string.slice(
			0,
			flatTerminators.reduce((p, c) => {
 ...
 */
export function acceptOnly<N extends Returnable>(
	parser: InfallibleParser<N>,
